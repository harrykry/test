...
-- depends_on: {{ source('line3_huaxuanmes3', '_airbyte_raw_uponline') }}
select
    {{ json_extract_scalar('_airbyte_data', ['UpOnlineTypeName'], ['UpOnlineTypeName']) }} as uponlinetypename,
    {{ json_extract_scalar('_airbyte_data', ['_ab_cdc_event_serial_no'], ['_ab_cdc_event_serial_no']) }} as _ab_cdc_event_serial_no,
    {{ json_extract_scalar('_airbyte_data', ['DZNum'], ['DZNum']) }} as dznum,
    ...
    {{ json_extract_scalar('_airbyte_data', ['UpOnlineID'], ['UpOnlineID']) }} as uponlineid,
    {{ json_extract_scalar('_airbyte_data', ['_ab_cdc_lsn'], ['_ab_cdc_lsn']) }} as _ab_cdc_lsn,
    {{ json_extract_scalar('_airbyte_data', ['SGJNum'], ['SGJNum']) }} as sgjnum,
    {{ json_extract_scalar('_airbyte_data', ['_ab_cdc_cursor'], ['_ab_cdc_cursor']) }} as _ab_cdc_cursor,
    {{ json_extract_scalar('_airbyte_data', ['XGJNum'], ['XGJNum']) }} as xgjnum,
    _airbyte_ab_id,
    _airbyte_emitted_at,
    {{ current_timestamp() }} as _airbyte_normalized_at
from {{ source('line3_huaxuanmes3', '_airbyte_raw_uponline') }} as table_alias
-- uponline
where 1 = 1
