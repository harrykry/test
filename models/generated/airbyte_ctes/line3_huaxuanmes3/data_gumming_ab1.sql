...
-- depends_on: {{ source('line3_huaxuanmes3', '_airbyte_raw_data_gumming') }}
select
    {{ json_extract_scalar('_airbyte_data', ['TunnelStoveTime'], ['TunnelStoveTime']) }} as tunnelstovetime,
    {{ json_extract_scalar('_airbyte_data', ['Temperature'], ['Temperature']) }} as temperature,
    {{ json_extract_scalar('_airbyte_data', ['GummingEndDateTime'], ['GummingEndDateTime']) }} as gummingenddatetime,
    {{ json_extract_scalar('_airbyte_data', ['_ab_cdc_event_serial_no'], ['_ab_cdc_event_serial_no']) }} as _ab_cdc_event_serial_no,
    {{ json_extract_scalar('_airbyte_data', ['GummingResult'], ['GummingResult']) }} as gummingresult,
...
from {{ source('line3_huaxuanmes3', '_airbyte_raw_data_gumming') }} as table_alias
-- data_gumming
where 1 = 1
