    ...
    post_hook = ["
                    ...
                    delete from {{ final_table_relation }} where {{ final_table_relation }}._airbyte_unique_key in (
                        ...
                        from (
                                ...
                                from {{ this }}
                                where 1=1 {{ incremental_clause('_airbyte_normalized_at', adapter.quote(this.schema) + '.' + adapter.quote('data_winding')) }}
                            ) recent_records
                            left join (
                                select _airbyte_unique_key as unique_key, count(_airbyte_unique_key) as active_count
                                from {{ this }}
                                where _airbyte_active_row = 1 {{ incremental_clause('_airbyte_normalized_at', adapter.quote(this.schema) + '.' + adapter.quote('data_winding')) }}
                    ...
                    {% endif %}
                    ","drop view _airbyte_line3_huaxuanmes3.data_winding_stg"],
    tags = [ "top-level" ]
) }}
-- depends_on: ref('data_winding_stg')
with
{% if is_incremental() %}
new_data as (
...
previous_active_scd_data as (
    ...
    select
        {{ star_intersect(ref('data_winding_stg'), this, from_alias='inc_data', intersect_alias='this_data') }}
...
input_data as (
    select {{ dbt_utils.star(ref('data_winding_stg')) }} from new_data
    union all
    select {{ dbt_utils.star(ref('data_winding_stg')) }} from previous_active_scd_data
),
{% else %}
input_data as (
    select *
